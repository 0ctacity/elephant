...
Export writes a deterministic versioned JSON envelope (format_version 1, no wall-clock fields): global entry IDs, actor IDs, lifecycle fields, project and source-Elephant metadata, entries sorted by ID, and relations sorted by from/type/to. Identical stored state serializes byte-identically. Import strictly decodes the archive (unknown fields and trailing data rejected), validates IDs, kinds, statuses, paths, relationships, and ownership crossings before committing anything, then stores the result in a separate `import:NAME` source table so local truth is never overwritten. Idempotency is keyed by archive identity plus the canonical content digest: repeating an identical import returns the existing source, while conflicting content under the same archive name fails explicitly. Failed imports roll back tables, rows, graph nodes and edges, and receipts together. Backup stages a storage-safe snapshot in a sibling temporary file and renames it into place only on success, preserving any existing destination on failure. MCP adds `export_project` and `import_project`. Format version 1 covers entries and relations; evidence, checkpoints, and adoption sections will ride a format bump when those features land.
