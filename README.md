...
Search is project-scoped (use `--remote SRC` for a received table), composes kind/status/actor/version/file/time filters across CLI and MCP, and returns stable ordering with match metadata. Commit filters form an inclusive Git commit range resolved against the local repository: `--commit-start`/`--commit-end` bound it and `--commit C` selects entries active at a single commit, matching entries whose recorded `start_commit`/`end_commit` interval intersects the range; a bound that does not resolve to a commit fails the query instead of degrading to an equality match. Graph traversal is bounded (direction, edge, depth 1–3, limit 1–50) and never crosses project or remote ownership. MCP adds `search_entries`, `related_entries`, and `file_history`.
