...
Search is project-scoped (use `--remote SRC` for a received table), composes kind/status/actor/version/file/time filters across CLI and MCP, and returns stable ordering with match metadata. Commit filters form an inclusive Git commit range resolved against the local repository: `--commit-start`/`--commit-end` bound it and `--commit C` selects entries active at a single commit, matching entries whose recorded `start_commit`/`end_commit` interval intersects the range; a bound that does not resolve to a commit fails the query instead of degrading to an equality match, and the start bound must be equal to or an ancestor of the end bound (reversed or divergent endpoints are rejected with a clear error). Graph traversal is bounded (direction, edge, depth 1–3, limit 1–50) and never crosses project or remote ownership. MCP adds `search_entries`, `related_entries`, and `file_history`.
...
Checkpoints record where one coding session stopped and the next should resume. They are scoped to the owning project table (local checkpoints never leak into remote recall) and carry actor provenance, Git start/end commits, and timestamps.
...
`checkpoint` accepts repeatable `--completed`, `--next`, `--commands`, and `--failures` list items (up to 50 per list, 2000 bytes each), repeatable `--file PATH`, repeatable `--relation TYPE:ENTRY_UUID`, and `--start-commit COMMIT`. An explicitly supplied start commit is verified with Git and rejected when unknown; otherwise the start defaults to the previous checkpoint's end commit, or to the current HEAD. The end commit is the current HEAD. Creation and all links happen in one transaction, so a failed link rolls back the checkpoint. MCP exposes `add_checkpoint` and `list_checkpoints`. `recall` includes the latest local checkpoint with its relations before older project context. Checkpoints live in the schema v4 `checkpoints` table.
...
Export writes a deterministic versioned JSON envelope (format_version 2, no wall-clock fields): global entry IDs, actor IDs, table ownership, lifecycle fields, project and source-Elephant metadata, entries sorted by ID, relations sorted by from/type/to, evidence sorted by entry/creation/ID, checkpoints in creation order, and adoption receipts sorted by source and source entry. Identical stored state serializes byte-identically. Import strictly decodes the archive (unknown fields and trailing data rejected), validates IDs, kinds, statuses, paths, relationships, ownership crossings, and rejects duplicate adoption receipts before committing anything, then stores the result in a separate `import:NAME` source table so local truth is never overwritten. Imported adoption receipts stay archived data under that table: they are never inserted into the live adoption registry, so they cannot satisfy, shadow, or otherwise alter a later local adoption, while remaining inspectable and countable on the archive. Idempotency is keyed by archive identity plus the canonical content digest: repeating an identical import returns the existing source, while conflicting content under the same archive name fails explicitly. Failed imports roll back tables, rows, graph nodes and edges, and receipts together. Backup stages a storage-safe snapshot in a sibling temporary file and renames it into place only on success, preserving any existing destination on failure. MCP adds `export_project` and `import_project`. Format version 2 covers the complete exported state: entries, relations, evidence, checkpoints, and adoption receipts. Format version 1 envelopes (entries and relations only) remain readable on import.
...
Review is deterministic and read-only. It reports stable codes (`MISSING_FILE`, `STALE_TASK`, `UNVERIFIED_FACT`, `DANGLING_RELATION`, `SUPERSESSION_ANOMALY`, `UNKNOWN_ACTOR`, `STALE_REMOTE`, `EVIDENCE_UNAVAILABLE`, `EVIDENCE_CHANGED`, `EVIDENCE_MISSING`), severity, source-separated entry context, and an explicit suggested action. Thresholds default to 30/90/30 days with bounded `--stale-task-days`, `--unverified-fact-days`, and `--stale-remote-days` flags. Fact verification follows evidence records rather than entry edits: a fact measures from its newest evidence `VerifiedAt`, or from its creation time when it has no evidence at all, so new facts are not flagged immediately. Every stored source of this project is reviewed — including imported archives without a registered remote — while source tables belonging to other projects never appear, and unavailable information (remote file references, unregistered peers, foreign baselines) is distinguished from confirmed inconsistency (changed or missing evidence). MCP adds `review_memory`.
